...
export default class HistogramPlot {
    constructor() {
        ...
        this.distanceData = new Map([['<1.25 Km', { 'Pub': 0, 'Restaurant': 0 }], ['<2.55 Km', { 'Pub': 0, 'Restaurant': 0 }], ['>2.55 Km', { 'Pub': 0, 'Restaurant': 0 }]])
    ...
    initChart(sel, data, isActivitiesView) {
        ...
        this.bars_wrapper = this.wrapper.append('g')
        ...
        if (!isActivitiesView) {
            ...
            // Aggregate the data for each distance category and venueType, summing on count
            this.distanceData.forEach((value, key) => {
                let sumPub = d3.sum(data, d => d.distanceCategory === key && d.venueType === 'Pub' ? d.count : 0)
                let sumRestaurant = d3.sum(data, d => d.distanceCategory === key && d.venueType === 'Restaurant' ? d.count : 0)
            ...
            let stackedData = d3.stack()
                .keys(['Restaurant', 'Pub'])(Array.from(this.distanceData, ([key, value]) => {
                    return {
                        key: key,
                        Pub: value.Pub,
    ...
    updateChart(participantsData) {
        if (!this.isActivitiesView) {
            ...
            // Update the text labels and their positions
            this.barsText.data(this.distanceData)
                .text(d => CONSTANTS.NUMBER_FORMATTER.format((d[1]?.Pub ?? 0) + (d[1]?.Restaurant ?? 0)))
                .transition()
                .duration(CONSTANTS.TRANSITION_DURATION)
